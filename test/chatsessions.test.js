...
test('registers opencode controller and serves content', async () => {
  ...
  const bridge = { listSessions: async () => [], newSession: async () => ({ resource: 'opencode://session/abc', label: 'abc' }), prompt: async (handle, text) => { prompts.push([handle, text]); }, cancel: async () => {}, dispose: () => {} };
  const prompts = [];
  ...
  const streamed = [];
  await content.requestHandler({ prompt: 'hello' }, {}, { markdown: (t) => { streamed.push(t); } }, {});
  assert.ok(streamed.length === 1 && streamed[0].includes('hello'), 'requestHandler echoes prompt to stream');
